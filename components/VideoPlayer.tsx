...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ embedUrl, onProgress, initialPercent = 0 }) => {
  ...
  // Đồng bộ Fullscreen cho Android/PC
  useEffect(() => {
  ...
  const handleFullscreen = async (e: React.MouseEvent) => {
    ...
    // Kiểm tra xem trình duyệt có hỗ trợ requestFullscreen trên div không (iOS iPhone thường trả về false)
    const supportsRequestFs = !!(container.requestFullscreen || (container as any).webkitRequestFullscreen);

    try {
      if (isFullscreen) {
        // Thoát Fullscreen
        if (supportsRequestFs) {
          const exitFs = document.exitFullscreen || (document as any).webkitExitFullscreen;
          if (exitFs) await exitFs.call(document);
        } else {
          // iOS Pseudo-FS: Chỉ cần set state
        ...
        if (supportsRequestFs) {
          ...
          // Xoay ngang nếu có thể
          if (window.screen.orientation && (window.screen.orientation as any).lock) {
            await (window.screen.orientation as any).lock('landscape').catch(() => {});
          }
        } else {
          // iOS iPhone Fallback: Sử dụng Pseudo-Fullscreen (CSS Fixed)
          setIsFullscreen(true);
          // Nếu người dùng muốn dùng player mặc định của iOS thay vì UI tùy chỉnh:
          // if ((video as any).webkitEnterFullscreen) (video as any).webkitEnterFullscreen();
        }
      }
    } catch (err) {
      console.error("Fullscreen Toggle Error:", err);
      // Fallback cuối cùng cho mọi thiết bị
      if ((video as any).webkitEnterFullscreen) (video as any).webkitEnterFullscreen();
    }
  };
  ...
  return (
    <div 
      ref={containerRef}
      className={`relative w-full aspect-video bg-black rounded-2xl overflow-hidden shadow-2xl group select-none transition-all duration-300 ${isFullscreen ? 'fixed inset-0 z-[9999] rounded-none w-screen h-screen' : ''}`}
      ...
      <div className={`absolute inset-0 z-40 flex flex-col justify-between bg-gradient-to-t from-black/95 via-transparent to-black/40 pointer-events-none transition-opacity duration-500 ${showControls || !isPlaying ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="p-3 md:p-5 flex items-center justify-between pointer-events-auto">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[8px] md:text-[10px] font-black text-white/50 tracking-widest uppercase">TuanPhim Media</span>
          </div>
          ...
          <div className="flex items-center justify-between pb-1 md:pb-0">
            ...
            <div className="flex items-center gap-1.5">
              <div className="relative">
                ...
                {showSettings && (
                  <div className="absolute bottom-full right-0 mb-2 w-20 bg-neutral-900 border border-white/10 rounded-xl p-1 flex flex-col gap-1 z-50">
